...
filter_train = lambda df: df.filter(pl.col('column_4') == 'valid')        # komt uit de repo
...
ds = GenomeIntervalDataset(
    ...
    bed_file = '/exports/humgen/idenhond/Enformer_data_google/data_human_sequences.bed',                   # bed file - columns 0, 1, 2 must be <chromosome>, <start position>, <end position>
...
# ik denk dat we dit niet nodig hebben nu, staat bij kopje fine tuning in github
# enformer = Enformer.from_pretrained('EleutherAI/enformer-official-rough')

# seq = torch.randint(0, 5, (4, 196_608)) # indicate number of sequences X with size argument (X, 196_608) # random sequence, niet nodig met fasta
# seq = ds[0] 
