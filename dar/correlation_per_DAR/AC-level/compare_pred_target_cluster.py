...
with open(f'/exports/humgen/idenhond/projects/dar/correlation_per_DAR/AC-level/Clusters_pred_target_ac_test.txt', 'w') as file:
    ...
    for i in range(pred_ac_test.shape[1]):
        ...
        # if i == 5: break
print(f'aantal goed AC test: {aantal_goed} van de {pred_ac_test.shape[1]}')

aantal_goed = 0 
with open(f'/exports/humgen/idenhond/projects/dar/correlation_per_DAR/AC-level/Clusters_pred_target_ac.txt', 'w') as file:
    file.truncate(0)
    header = ['nr', 'target', 'prediction', '\n']
    file.write('\t'.join(header))
    for i in range(pred_ac.shape[1]):
        pred_values = pred_ac[:, i]
        target_values = target_ac[:, i]
        ...
        if class_pred == class_target: aantal_goed += 1
        # if i == 5: break
print(f'aantal goed AC: {aantal_goed} van de {pred_ac.shape[1]}')
