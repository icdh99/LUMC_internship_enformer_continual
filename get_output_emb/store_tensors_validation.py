...
df_valid = df[df['category'] == 'valid'].reset_index(drop=True)
print(f'number of validation sequences: {df_valid.shape}')
if not os.path.exists('/exports/humgen/idenhond/enformer_dev/enformer-pytorch/scripts/tmp_bed_val'):
    os.mkdir('tmp_bed_val')
    print('directory tmp_bed_val is created')
else: 
    print('directory tmp_bed_val already exists')
...
"""
store output and embeddings for each validation sequence seperately
"""
...
for row in df_valid.itertuples():
    ...
    with open('tmp_bed_val/tmp_val.bed', 'w') as f:
        f.truncate(0)
        line = [row.chr, str(row.start), str(row.end), 'valid', '\n']
        f.write('\t'.join(line))

    # load sequences
    ds = GenomeIntervalDataset(
        bed_file = 'tmp_bed_val/tmp_val.bed',
    ...
    torch.save(embeddings, f'/exports/humgen/idenhond/enformer_dev/enformer-pytorch/data/tensors_validation/embeddings_seq{t}.pt')
    torch.save(output, f'/exports/humgen/idenhond/enformer_dev/enformer-pytorch/data/tensors_validation/output_seq{t}.pt')
