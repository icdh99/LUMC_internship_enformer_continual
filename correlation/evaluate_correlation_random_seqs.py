...
"""
random seq:
seq = torch.randint(0, 5, (x, 196_608))
x is number of sequences (2d shape)
...
def compute_correlation(model, organism:str="human", subset:str=subset, max_steps=max_steps):
  ...
  for i,batch in enumerate(tqdm(dl, total=n_steps)):
    ...
    replace with random sequence
    x = 1 --> 1 random sequence
    """
    sequence = torch.randint(0, 5, (1, 196_608)).cuda()
  ...
  df.to_csv(f"/exports/humgen/idenhond/data/evaluate_correlation/correlation_per_track_{subset}_random_seqs.csv",index=True)
