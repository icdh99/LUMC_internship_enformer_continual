...
class model(pl.LightningModule):
	def __init__(self):
		...
		self.linear = nn.Linear(in_features = 3072, out_features = 5332, bias = True)
...
# load model trained on new tracks only
path = '/exports/humgen/idenhond/projects/enformer/dnn_head/train_alltracks_3003/model_2023-03-30 15:03:10.553990/epoch=19-step=5320-val_loss=0.8.ckpt'
print(path)
...
def compute_correlation(model, organism:str="human", subset:str=subset, max_steps=max_steps):
  ...
  corr_coef = MeanPearsonCorrCoefPerChannel(n_channels=5332) # 5313 channels
  ...
  for i,batch in enumerate(tqdm(dl, total=n_steps)):
    ...
    with torch.no_grad():
      if subset == 'train': 
          ...
          # target = torch.load(f'/exports/humgen/idenhond/data/Enformer_train/Enformer_train_targets_newtracks2703/targets_seq{i+1}.pt', map_location=torch.device(device))
          y_oldtracks = torch.load(f'/exports/humgen/idenhond/data/Enformer_train/Enformer_train_targets/targets_seq{i+1}.pt', map_location=torch.device(device)).squeeze()
          y_newtracks = torch.load(f'/exports/humgen/idenhond/data/Enformer_train/Enformer_train_targets_newtracks2703/targets_seq{i+1}.pt', map_location=torch.device(device)).squeeze()
          target = torch.cat((y_oldtracks, y_newtracks), dim = 1)
          target = torch.unsqueeze(target, 0)
      elif subset == 'test': 
          emb = tensor_out[i].cuda()
          target = torch.load(f'/exports/humgen/idenhond/data/Enformer_test/Enformer_test_targets_newtracks2703/targets_seq{i+1}.pt', map_location=torch.device(device))
          y_oldtracks = torch.load(f'/exports/humgen/idenhond/data/Enformer_test/Enformer_test_targets_perseq/targets_seq{i+1}.pt', map_location=torch.device(device)).squeeze()
          y_newtracks = torch.load(f'/exports/humgen/idenhond/data/Enformer_test/Enformer_test_targets_newtracks2703/targets_seq{i+1}.pt', map_location=torch.device(device)).squeeze()
          target = torch.cat((y_oldtracks, y_newtracks), dim = 1)
          target = torch.unsqueeze(target, 0)
      elif subset == 'valid': 
          ...
          # target = torch.load(f'/exports/humgen/idenhond/data/Enformer_validation/Enformer_validation_targets_newtracks2703/targets_seq{i+1}.pt', map_location=torch.device(device))
          y_oldtracks = torch.load(f'/exports/humgen/idenhond/data/Enformer_validation/Enformer_validation_targets_perseq/targets_seq{i+1}.pt', map_location=torch.device(device)).squeeze()
          y_newtracks = torch.load(f'/exports/humgen/idenhond/data/Enformer_validation/Enformer_validation_targets_newtracks2703/targets_seq{i+1}.pt', map_location=torch.device(device)).squeeze()
          target = torch.cat((y_oldtracks, y_newtracks), dim = 1)
          target = torch.unsqueeze(target, 0)
      ...
      # if subset == 'train': torch.save(pred, f'/exports/humgen/idenhond/data/Enformer_train/Enformer_train_output_dnnhead_alltracks/output_seq{i+1}.pt')
      # if subset == 'test': torch.save(pred, f'/exports/humgen/idenhond/data/Enformer_test/Enformer_test_output_dnnhead_alltracks/output_seq{i+1}.pt')
      # if subset == 'valid': torch.save(pred, f'/exports/humgen/idenhond/data/Enformer_validation/Enformer_validation_output_dnnhead_alltracks/output_seq{i+1}.pt')
...
# BESTANDSNAAM AANPASSEN
  ...
  df.to_csv(f"/exports/humgen/idenhond/data/evaluate_correlation/correlation_per_track_{subset}_dnn_head_alltracks.csv",index=True)
