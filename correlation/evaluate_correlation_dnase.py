...
# CLASS MODEL NR OUT FEATURES = 674 = DNASE ONLY
class model(pl.LightningModule):
	def __init__(self):
		# define model
		super(model, self).__init__()
		self.linear = nn.Linear(in_features = 3072, out_features = 674, bias = True)
...
path = '/exports/humgen/idenhond/projects/enformer/dnn_head/train_dnase/model_2023-03-28 11:23:49.741105/epoch=16-step=4522-val_loss=1.0.ckpt' # dnase only model, 31/3
print(path)
...
def compute_correlation(model, organism:str="human", subset:str=subset, max_steps=max_steps):
  ...
  corr_coef = MeanPearsonCorrCoefPerChannel(n_channels=674) # 5313 channels
  ...
  for i,batch in enumerate(tqdm(dl, total=n_steps)):
    ...
    with torch.no_grad():
      ...
      if subset == 'test': emb = tensor_out[i].cuda() 
  ...
  df.to_csv(f"/exports/humgen/idenhond/data/evaluate_correlation/correlation_per_track_{subset}_dnase_3103.csv",index=True)
