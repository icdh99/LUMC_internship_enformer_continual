...
class BasenjiDataSet(torch.utils.data.IterableDataset):
  ...
  def get_tfrecord_files(cls, organism, subset):
    ...
    path_to_tfr_records = f'/exports/humgen/idenhond/data/basenji_preprocess/output_tfr_human_atac_normalized/tfrecords/{subset}-*.tfr'
    # if subset == 'train':
...
path = '/exports/humgen/idenhond/projects/enformer/dnn_head/train_human_atac_normalized/model_2023-04-28 11:16:53.260811/epoch=15-step=4256-val_loss=1.0.ckpt'
print(path)
  ...
  for i,batch in enumerate(tqdm(dl, total=n_steps)):
    ...
    with torch.no_grad():
      if subset == 'train': 
          ...
          target = torch.load(f'/exports/humgen/idenhond/data/Enformer_train/Human_ATAC_normalized_train_targets/targets_seq{i+1}.pt', map_location=torch.device(device))
      elif subset == 'test': 
          emb = tensor_out[i].cuda()
          target = torch.load(f'/exports/humgen/idenhond/data/Enformer_test/Human_ATAC_normalized_test_targets/targets_seq{i+1}.pt', map_location=torch.device(device))
      elif subset == 'valid': 
          ...
          target = torch.load(f'/exports/humgen/idenhond/data/Enformer_validation/Human_ATAC_normalized_validation_targets/targets_seq{i+1}.pt', map_location=torch.device(device))
    #   print(f'emb shape: {emb.shape}')
    #   print(f'target shape: {target.shape}')
      pred1 = model(emb)
      pred = torch.unsqueeze(pred1, 0)
    #   print(f'pred shape: {pred.shape}')
      if subset == 'train': torch.save(pred, f'/exports/humgen/idenhond/data/Enformer_train/Enformer_train_output_humanatac_normalized/output_seq{i+1}.pt')
      if subset == 'test': torch.save(pred, f'/exports/humgen/idenhond/data/Enformer_test/Enformer_test_output_humanatac_normalized/output_seq{i+1}.pt')
      if subset == 'valid': torch.save(pred, f'/exports/humgen/idenhond/data/Enformer_validation/Enformer_validation_output_humanatac_normalized/output_seq{i+1}.pt')
...
# BESTANDSNAAM AANPASSEN
  ...
  df.to_csv(f"/exports/humgen/idenhond/data/evaluate_correlation/correlation_per_track_{subset}_humanatac_normalized.csv",index=True)
