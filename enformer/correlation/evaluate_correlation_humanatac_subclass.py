...
class model(pl.LightningModule):
	def __init__(self):
		...
		self.linear = nn.Linear(in_features = 3072, out_features = 20, bias = True) #ADJUST NUMBER OF OUTPUT FEATURES
...
path = '/exports/humgen/idenhond/projects/enformer/dnn_head/train_human_atac_Subclass/model_2023-05-05 23:31:10.425939/epoch=19-step=5320-val_loss=0.4.ckpt'
print(path)
...
def compute_correlation(model, organism:str="human", subset:str=subset, max_steps=max_steps):
  ...
  corr_coef = MeanPearsonCorrCoefPerChannel(n_channels=20) # ADJUST NUMBER OF CHANNELS TO NR OF TRACKS
  ...
  for i,batch in enumerate(tqdm(dl, total=n_steps)):
    ...
    with torch.no_grad():
      if subset == 'train': 
          ...
          idx_in = [51, 60, 61, 62, 63, 64, 43, 45, 46, 44, 48, 49, 50, 47, 0, 4, 53, 55, 58, 65]
    ...
    #   print(f'pred shape: {pred.shape}')
      ...
      if subset == 'test': torch.save(pred, f'/exports/humgen/idenhond/data/Enformer_test/Enformer_test_output_humanatac_subclass/output_seq{i+1}.pt')
...
# BESTANDSNAAM AANPASSEN
  ...
  df.to_csv(f"/exports/humgen/idenhond/data/evaluate_correlation/correlation_per_track_{subset}_humanatac_subclass.csv",index=True)
