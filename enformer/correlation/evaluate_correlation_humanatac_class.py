...
class model(pl.LightningModule):
	def __init__(self):
		...
		self.linear = nn.Linear(in_features = 3072, out_features = 3, bias = True) #ADJUST NUMBER OF OUTPUT FEATURES
...
path = '/exports/humgen/idenhond/projects/enformer/dnn_head/train_human_atac_Class/model_2023-05-05 12:15:44.459115/epoch=17-step=4788-val_loss=0.6.ckpt'
print(path)
...
def compute_correlation(model, organism:str="human", subset:str=subset, max_steps=max_steps):
  ...
  corr_coef = MeanPearsonCorrCoefPerChannel(n_channels=3) # ADJUST NUMBER OF CHANNELS TO NR OF TRACKS
  ...
  for i,batch in enumerate(tqdm(dl, total=n_steps)):
    ...
    #   print(f'pred shape: {pred.shape}')
      ...
      if subset == 'test': torch.save(pred, f'/exports/humgen/idenhond/data/Enformer_test/Enformer_test_output_humanatac_class/output_seq{i+1}.pt')
...
# BESTANDSNAAM AANPASSEN
  ...
  df.to_csv(f"/exports/humgen/idenhond/data/evaluate_correlation/correlation_per_track_{subset}_humanatac_class.csv",index=True)
