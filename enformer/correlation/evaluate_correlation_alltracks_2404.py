...
class model(pl.LightningModule):
	def __init__(self):
		...
		self.linear = nn.Linear(in_features = 3072, out_features = 5340, bias = True) #ADJUST NUMBER OF OUTPUT FEATURES
...
class BasenjiDataSet(torch.utils.data.IterableDataset):
  ...
  def get_tfrecord_files(cls, organism, subset):
    ...
    path_to_tfr_records = f'/exports/humgen/idenhond/data/basenji_preprocess/output_tfr_27newtracks_human/tfrecords/{subset}-*.tfr'
    # if subset == 'train':
...
# load model trained on 27 new tracks
path = '/exports/humgen/idenhond/projects/enformer/dnn_head/train_alltracks_2404/model_2023-04-27 01:39:15.772161/epoch=19-step=5320-val_loss=0.8.ckpt'
print(path)
...
def compute_correlation(model, organism:str="human", subset:str=subset, max_steps=max_steps):
  ...
  corr_coef = MeanPearsonCorrCoefPerChannel(n_channels=5340) # ADJUST NUMBER OF CHANNELS TO NR OF TRACKS
  ...
  for i,batch in enumerate(tqdm(dl, total=n_steps)):
    ...
    with torch.no_grad():
      if subset == 'train': 
          ...
          target_old = torch.load(f'/exports/humgen/idenhond/data/Enformer_train/Enformer_train_targets/targets_seq{i+1}.pt', map_location=torch.device(device)).squeeze()
          target_new = torch.load(f'/exports/humgen/idenhond/data/Enformer_train/Newtracks_2404_train_targets/targets_seq{i+1}.pt', map_location=torch.device(device)).squeeze()
          target = torch.cat((target_old, target_new), dim = 1)
          target = torch.unsqueeze(target, 0)
      elif subset == 'test': 
          emb = tensor_out[i].cuda()
          target_old = torch.load(f'/exports/humgen/idenhond/data/Enformer_test/Enformer_test_targets_perseq/targets_seq{i+1}.pt', map_location=torch.device(device)).squeeze()
          target_new = torch.load(f'/exports/humgen/idenhond/data/Enformer_test/Newtracks_2404_test_targets/targets_seq{i+1}.pt', map_location=torch.device(device)).squeeze()
          target = torch.cat((target_old, target_new), dim = 1)
          target = torch.unsqueeze(target, 0)
      elif subset == 'valid': 
          ...
          target_old = torch.load(f'/exports/humgen/idenhond/data/Enformer_validation/Enformer_validation_targets_perseq/targets_seq{i+1}.pt', map_location=torch.device(device)).squeeze()
          target_new = torch.load(f'/exports/humgen/idenhond/data/Enformer_validation/Newtracks_2404_validation_targets/targets_seq{i+1}.pt', map_location=torch.device(device)).squeeze()
          target = torch.cat((target_old, target_new), dim = 1)
          target = torch.unsqueeze(target, 0)
    ...
    #   print(f'pred shape: {pred.shape}')
      # if subset == 'train': torch.save(pred, f'/exports/humgen/idenhond/data/Enformer_train/Enformer_train_output_newtracks_2404/output_seq{i+1}.pt')
      # if subset == 'test': torch.save(pred, f'/exports/humgen/idenhond/data/Enformer_test/Enformer_test_output_newtracks_2404/output_seq{i+1}.pt')
      # if subset == 'valid': torch.save(pred, f'/exports/humgen/idenhond/data/Enformer_validation/Enformer_validation_output_newtracks_2404/output_seq{i+1}.pt')
...
# BESTANDSNAAM AANPASSEN
  ...
  df.to_csv(f"/exports/humgen/idenhond/data/evaluate_correlation/correlation_per_track_{subset}_alltracks_2404.csv",index=True)
