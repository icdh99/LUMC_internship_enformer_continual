...
class model(pl.LightningModule):
	def __init__(self):
		...
		self.linear = nn.Linear(in_features = 3072, out_features = 27, bias = True) #ADJUST NUMBER OF OUTPUT FEATURES
...
path = '/exports/humgen/idenhond/projects/enformer/dnn_head/train_newtracks_2404/model_2023-04-24 19:42:56.514076/epoch=18-step=5054-val_loss=0.8.ckpt'
print(path)
...
def compute_correlation(model, organism:str="human", subset:str=subset, max_steps=max_steps):
  ...
  corr_coef = MeanPearsonCorrCoefPerChannel(n_channels=27) # ADJUST NUMBER OF CHANNELS TO NR OF TRACKS
  ...
  for i,batch in enumerate(tqdm(dl, total=n_steps)):
    ...
    with torch.no_grad():
      if subset == 'train': 
          ...
          target = torch.load(f'/exports/humgen/idenhond/data/Enformer_train/Newtracks_2404_train_targets/targets_seq{i+1}.pt', map_location=torch.device(device))
      elif subset == 'test': 
          emb = tensor_out[i].cuda()
          target = torch.load(f'/exports/humgen/idenhond/data/Enformer_test/Newtracks_2404_test_targets/targets_seq{i+1}.pt', map_location=torch.device(device))
      elif subset == 'valid': 
          ...
          target = torch.load(f'/exports/humgen/idenhond/data/Enformer_validation/Newtracks_2404_validation_targets/targets_seq{i+1}.pt', map_location=torch.device(device))
...
# BESTANDSNAAM AANPASSEN
  ...
  df.to_csv(f"/exports/humgen/idenhond/data/evaluate_correlation/correlation_per_track_{subset}_newtracks_2404.csv",index=True)
